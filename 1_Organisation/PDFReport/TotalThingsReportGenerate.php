...
abstract class PDFReport extends FPDF
{
	...
	{
		$this->Output("D","Things Report".$this->today_date);
		//$this->Output();
	}
	...
	public function Header()
	{
		...
		$this->Ln();
		$this->SetFont('Arial','',14); 
		$this->MultiCell(170,8," Things Report \n".$this->title,1,'C');
		$this->Ln(5);
...
class PDFClassStudentReport extends PDFReport
{
	...
	{
		parent::__construct();
		$this->col_width = array(50,120);
		$this->header = array('Id', 'Thing Name');
		...
		foreach($ThingData_object_array as $ThingData_object)
		{
			...
			$i=0;
			$this->Cell($this->col_width[$i++],7,$ThingData_object->ID,1);
			$this->Cell($this->col_width[$i++],7,$ThingData_object->NAME,1);
			$this->Ln();
