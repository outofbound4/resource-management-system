...
abstract class PDFReport extends FPDF
{
	...
	public function Header()
	{
		...
		$this->Ln();
		$this->SetFont('Arial','',14); 
		$this->MultiCell(170,8," Thing Maintanance Report \n".$this->title,1,'C');
		$this->Ln(5);
...
class PDFClassStudentReport extends PDFReport
{
	...
	public function __construct($class_id)
	{
		...
		$this->header = array('Id', 'Thing Name','Repair Name','Cost(in Rs.)');
		...
		foreach($ThingMaintananceData_object_array as $ThingMaintananceData_object)
		{
			...
			$i=0;
			$this->Cell($this->col_width[$i++],7,$ThingMaintananceData_object->ID,1);
			$this->Cell($this->col_width[$i++],7,$ThingMaintananceData_object->ThingData_object->NAME,1);
			$this->Cell($this->col_width[$i++],7,$ThingMaintananceData_object->NAME,1);
			$this->Cell($this->col_width[$i++],7,$ThingMaintananceData_object->COST,1);
			$this->Ln();
